...
colinear <- function(z1, z2, z3, givepos=F){
  lambda <- (z1-z3)/(z3-z2)
  if (givepos){
    return(ifelse(lambda==0, 0, lambda/abs(lambda)))
  }
  return(Im(lambda)==0) 
}
...
lineintercept <- function (Z1,Z2,Z3,Z4, segment = T, test.lambda=F){
  ...
  if (segment) {
    ...
    lambda2 <- -((X - P3[1]) * dx2 + (Y - P3[2]) * dy2)/(dx2^2 + 
      dy2^2)
    lambda1 <- round(lambda1,digits=8)
    lambda2 <- round(lambda2, digits=8)
